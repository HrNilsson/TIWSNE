...
class ReliableMoteIF extends MoteIF {
	...
	public ReliableMoteIF(PhoenixSource base) {
		...
		super.registerListener(new AckMsg(), new MessageListener() {
	...
	public void send(int to, Message m) throws IOException {
		synchronized (ackLock) {
			/*
			 * Build a reliable_msg_t packet with the current cookie and the
			 * payload in m, total packet size is 'm.dataLength() +
			 * offsetUserData'
			 */
			ReliableMsg rmsg = new ReliableMsg(m.dataLength() + offsetUserData);
			rmsg.set_cookie(sendCookie);
			System.arraycopy(m.dataGet(), m.baseOffset(), rmsg.dataGet(),
					offsetUserData, m.dataLength());

			/*
			 * Repeatedly transmit 'rmsg' until the ack handler tells us an ack
			 * is received.
			 */
			acked = false;
			for (;;) {
	...
	public void registerListener(Message m, MessageListener l) {
		 ...
		 */
		super.registerListener(new ReliableMsg(), new MessageListener() {
			public void messageReceived(int to, Message m) {
	...
	void reliableMsgReceived(int to, ReliableMsg rmsg) {
		...
		if (rmsg.get_cookie() != recvCookie) {
			...
			/*
			 * Extract payload from 'rmsg' and copy it into a copy of
			 * 'template'. The payload is all the data in 'rmsg' from
			 * 'offsetUserData' on
			 */
			Message userMsg = template
					.clone(rmsg.dataLength() - offsetUserData);
			System.arraycopy(rmsg.dataGet(),
					rmsg.baseOffset() + offsetUserData, userMsg.dataGet(), 0,
