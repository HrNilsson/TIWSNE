...
module ReliableSerialC {
	provides {
		interface AMSend;
		interface Receive;
		interface Set<bool> as FlowControl;
	}
	uses {
...
implementation {
	...
	void done(error_t ok) {
		...
		sending = NULL;
		sendCookie *= 3;
		/* New cookie time! Cycles every 64 cookies. */
	...
	command error_t AMSend.send(am_addr_t addr, message_t * msg, uint8_t len) {
		...
		if( ! sending) {
			reliable_msg_t * header = call SubSend.getPayload(msg,
					sizeof(reliable_msg_t));
	...
	event void Timer.fired() {
		error_t ok = call SubSend.send(sendAddr, sending, sendLen 
				+ sizeof(reliable_msg_t));
		if(ok != SUCCESS) 
			done(ok);
	}

	event message_t * AckReceive.receive(message_t * m, void * payload,
			uint8_t len) {
	...
	event message_t * SubReceive.receive(message_t * m, void * payload,
			uint8_t len) {
	...
	default event message_t * Receive.receive(message_t * m, void * payload,
			uint8_t len) {
		return m;
	}
	...
	command void * AMSend.getPayload(message_t * msg, uint8_t len) {
		reliable_msg_t * header = call SubSend.getPayload(msg, len 
				+ sizeof(reliable_msg_t));

		return header ? header + 1 : NULL;
	}

	command void FlowControl.set(bool val) {
		flow = val;
	}
}